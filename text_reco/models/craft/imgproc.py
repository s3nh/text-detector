...
class ImageConvert():
    ...
    def normalizeMeanVariance(self):
        self.image -= np.array([self.mean[0]  * 255.0, self.mean[1] * 255.0, self.mean[2] * 255.0], dtype = np.float32)
        self.image /= np.array([variance[0] * 255.0, variance[1] * 255.0, variance[2] * 255.0], dtype = np.float32)
        return self.image
